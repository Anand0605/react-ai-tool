...
function App() {
  ...
  const [chatHistory, setChatHistory] = useState([]); // for main chat (user + ai)
  const [sidebarHistory, setSidebarHistory] = useState([]); // for sidebar (user only)
  ...
  const askQuestion = async () => {
    ...
    // Add to both main and sidebar history
    setChatHistory(prev => [...prev, userMessage]);
    setSidebarHistory(prev => [...prev, userMessage]);
    setQuestion('');
    setLoading(true);
  ...
  const clearHistory = () => {
    setSidebarHistory([]); // Only clears sidebar
  };
        ...
        <div className="grid grid-cols-5 h-screen text-center relative">
          ...
          >
            <div
              ...
              className="w-full flex flex-col space-y-4 overflow-auto mb-4 px-4 ml-[15px] max-h-[calc(100vh-100px)] hide-scrollbar"
            >
